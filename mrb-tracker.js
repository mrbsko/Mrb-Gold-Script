...
(function () {
  ...
  function omertaDate(ts) {
    // Omerta gameklok loopt 2 uur achter op de lokale NL-browserklok.
    // Alleen de zichtbare weergave wordt gecorrigeerd; opgeslagen timestamps blijven intact.
  ...
  function fmtTime(ts) {
    if (!ts) return '-';
    return omertaDate(ts).toLocaleTimeString('nl-NL', {hour:'2-digit',minute:'2-digit',second:'2-digit'});
  }
